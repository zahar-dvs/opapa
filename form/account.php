...
if($Module == 'edit' and $_POST['enter']){
    ...
    if($_FILES['avatar']['tmp_name']){
        ...
        if($_FILES['avatar']['size'] > 20000){
            MessageSend(2, 'Размер изображения слишком большой');
        }
...
if($Module == 'restore' and !$Param['code'] and substr($_SESSION['RESTORE'], 0, 4) == 'wait'){
    MessageSend(2, 'Вы уже отправили заявку на восстановление пароля. Проверьте ваш Email <b>'.HideEmail(substr($_SESSION['RESTORE'], 5)).'</b>');
}

if($Module == 'restore' and $_SESSION['RESTORE'] and substr($_SESSION['RESTORE'], 0, 4) != 'wait'){
    MessageSend(2, 'Ваш пароль уже был изменен. Для входа используйте новый пароль <b>'.$_SESSION['RESTORE'].'</b>', '/login');
}
...
if($Module == 'restore' and $_POST['enter']){
    ...
    if(!$_POST['login'] or !$_POST['captcha']){
        MessageSend(1, 'Ошибка обработки формы');
    }
    if ($_SESSION['captcha'] != md5($_POST['captcha'])){
        MessageSend(1, "Капча не совпадает");
    }
    ...
    mail($Row['email'], 'OPAPA site', 'Ссылка для восстановления: http://opapa.zz.mu/account/restore/code/'.md5($Raw['email']).$Row['id'], 'From: opapa.zz.mu');
    $_SESSION['RESTORE'] = 'wait_'.$Row['email'];
    MessageSend(3, 'На ваш Email адрес <b>'.HideEmail($Row['email']).'</b> отправлено сообщение подтверждения смены пароля');
}
...
if ($Module == 'register' and $_POST['enter']){
    ...
    if (!$_POST['login'] or !$_POST['email'] or !$_POST['password'] or !$_POST['name'] or $_POST['country'] > 4 or !$_POST['captcha']){
        MessageSend(1, 'Ошибка регистрации');
    }
    ...
    mysqli_query($CONNECT, "INSERT INTO `users`  VALUES (NULL , '$_POST[login]', '$_POST[password]', '$_POST[name]', NOW(), '$_POST[email]', $_POST[country], 0, 0)");
    $Code = str_replace('=', '', base64_encode($_POST['email']));
    mail($_POST['email'], 'Регистация в OPAPA site', 'Ссылка для активации: http://opapa.zz.mu/account/activate/code/'.substr($Code, -5).substr($Code, 0, -5), 'From: opapa.zz.mu');
    ...
    if(!$_SESSION['USER_ACTIVE_EMAIL']){
        ...
        if(strpos($Email, '@') !== false){
            ...
            MessageSend(3, 'Email адрес <b>'.$Email.'</b> подтвержден. Можете войти под своим логином. Спасибо за регистрацию!', '/login');
        }else MessageSend(1, 'Email адрес не подтвержден.', '/login');
    }else MessageSend(3, 'Email адрес <b>'.$_SESSION['USER_ACTIVE_EMAIL'].'</b> уже подтвержден.', '/login');
}
